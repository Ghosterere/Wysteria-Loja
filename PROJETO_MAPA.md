...
| `/painel_loja` | `store_panel` em `bot.py` | Central gráfica exclusiva do lojista | Slash command |
...
| Painel do vendedor | `OwnerShopBrowserView` em `bot.py` | Lista lojas ativas/arquivadas e permite criar loja | `/painel_loja` |
| Gestão da loja | `OwnerShopManageView` em `bot.py` | Centraliza personalização, catálogo, histórico, estatísticas e publicação | `/painel_loja` -> selecionar loja |
| Gestão do catálogo | `OwnerProductManageView` em `bot.py` | Cria, edita, ativa, desativa e remove produtos | `/painel_loja` → `Gerenciar catálogo` |
| Histórico da loja | `OwnerShopHistoryView` em `bot.py` | Mostra pedidos da loja e acesso aos logs | `/painel_loja` → `Histórico` |
| Estatísticas do editor | `EditorStatsView` em `bot.py` | Mostra total de pedidos, concluídos e tempo médio | `/painel_loja` → `Estatísticas` |
| Painel público da loja | `PublicPublishedShopView` em `bot.py` | Post público sincronizado com botão para abrir a loja | `/painel_loja` -> `Divulgar loja` |
...
| `OwnerShopBrowserView` | `bot.py` | Lista de lojas do lojista | `/painel_loja` |
| `OwnerShopManageView` | `bot.py` | Gestão central da loja | `/painel_loja` |
| `OwnerProductManageView` | `bot.py` | Gestão do catálogo | `/painel_loja` |
| `OwnerShopHistoryView` | `bot.py` | Histórico da loja | `/painel_loja` -> `Historico` |
| `EditorStatsView` | `bot.py` | Estatísticas do editor | `/painel_loja` -> `Estatisticas` |
| `ThemePresetView` | `bot.py` | Escolha de tema pronto | `/painel_loja` -> `Tema visual` |
...
| `PublishShopView` | `bot.py` | Seleção do canal da divulgação | `/painel_loja` -> `Divulgar loja` |

## Modals

| Nome | Localização no código | Função | Como acessar |
| --- | --- | --- | --- |
| `CreateShopModal` | `bot.py` | Cria loja pela interface | `/painel_loja` -> `Criar loja` |
| `ShopStyleModal` | `bot.py` | Ajusta descrição, emoji, cor e mídia | `/painel_loja` -> `Personalizar visual` |
| `ShopBioModal` | `bot.py` | Ajusta headline, subtítulo, vantagens e botão | `/painel_loja` -> `Ajustar vitrine` |
| `ShopTermsModal` | `bot.py` | Ajusta termos da loja | `/painel_loja` -> `Termos da loja` |
| `ShopStatusModal` | `bot.py` | Ajusta aberta/fechada e disponibilidade | `/painel_loja` -> `Status da loja` |
| `ProductCreateModal` | `bot.py` | Cria produto | `/painel_loja` -> `Novo serviço` |
| `ProductEditModal` | `bot.py` | Edita produto | `/painel_loja` -> `Gerenciar catálogo` |
...
| Dono da loja | Validações em `bot.py` | Só o dono edita loja, produtos e status | Painel `/painel_loja` |
